...
Semisupervise_AD_Pool = ['Left_STAMPi', 'SAND', 'MCD', 'Sub_MCD', 'OCSVM', 'Sub_OCSVM', 'AutoEncoder', 'CNN', 'LSTMAD', 'TranAD', 'USAD', 'OmniAnomaly', 
                        'AnomalyTransformer', 'TimesNet', 'FITS', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'StreamVAE', 'AxonAD', 'WVAE', 'VSVAE', 'VASP', 
                        'TFTResidual', 'SISVAE', 'MAVAE', 'GDN']
...
def run_StreamVAE(
        data_train, data_test, win_size=100, latent_dim=64,
        batch_size=128, epochs=30, patience=10,
        lr=0.001, validation_size=0.2, target_kl=100.0
    ):
...
def run_WVAE(
        data_train, data_test, win_size=100, latent_dim=5,
        batch_size=128, epochs=50, patience=10,
        lr=1e-3, noise_std=0.8, validation_size=0.2
    ):
    from .models.WVAE import WVAE
    clf = WVAE(
        win_size=win_size,
        feats=data_test.shape[1],
        latent_dim=latent_dim,
...
def run_VSVAE(
        data_train, data_test, win_size=100, latent_dim=3,
        attn_vec_size=3, batch_size=128, epochs=50,
        patience=10, lr=1e-3, validation_size=0.2
    ):
    from .models.VSVAE import VS_VAE
    clf = VS_VAE(
        win_size=win_size,
        feats=data_test.shape[1],
        latent_dim=latent_dim,
...
def run_VASP(
        data_train, data_test, win_size=100, latent_dim=8,
        batch_size=128, epochs=50, patience=10,
        lr=1e-3, kl_weight=0.5, validation_size=0.2
    ):
    from .models.VASP import VASP
    clf = VASP(
        win_size=win_size,
        feats=data_test.shape[1],
        latent_dim=latent_dim,
...
def run_TFTResidual(
        data_train, data_test, win_size=100, pred_len=10,
        d_model=64, d_hidden=128, n_heads=4, num_attn_layers=1,
        dropout=0.1, batch_size=128, epochs=30, lr=1e-3,
        validation_size=0.2, use_time_covariates=True, time_num_freqs=2
    ):
    from .models.TFTResidual import TFTResidual
    clf = TFTResidual(
        win_size=win_size,
        feats=data_test.shape[1],
        pred_len=pred_len,
...
def run_SISVAE(
        data_train, data_test, win_size=100, latent_dim=40,
        hidden_dim=200, batch_size=128, epochs=50,
        patience=10, lr=1e-3, smooth_weight=0.5, validation_size=0.2
    ):
    from .models.SISVAE import SISVAE
    clf = SISVAE(
        win_size=win_size,
        feats=data_test.shape[1],
        latent_dim=latent_dim,
...
def run_MAVAE(
        data_train, data_test, win_size=100, latent_dim=64,
        batch_size=128, epochs=50, patience=10,
        lr=1e-3, n_heads=8, noise_std=0.01, validation_size=0.2
    ):
    from .models import MAVAE
    from importlib import import_module
    # Handle the hyphenated filename
    mavae_module = import_module('.models.MAVAE', package='TSB_AD')
    MAVAE = mavae_module.MAVAE
...
def run_GDN(
        data_train, data_test, win_size=100, hidden_dim=128,
        layers=2, topk=15, tau=0.1, emb_dim=64,
        batch_size=128, epochs=30, patience=10,
        lr=1e-3, validation_size=0.2
    ):
