...
class ComplexLinearExact(nn.Module):
    """Mathematically identical replacement for nn.Linear().to(torch.cfloat)
    that works on Apple Silicon MPS by using real-valued kernels.
    """
    ...
    def __init__(self, seq_len, pred_len, individual, enc_in, cut_freq):
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.freq_upsampler.append(ComplexLinearExact(self.dominance_freq, int(self.dominance_freq*self.length_ratio)))

        else:
            self.freq_upsampler = ComplexLinearExact(self.dominance_freq, int(self.dominance_freq*self.length_ratio)) # complex layer for frequency upcampling
    ...
    def decision_function(self, data):
        ...
        with torch.inference_mode():
            for idx, (x, target) in loop:
                ...
                scores.append(score.detach().to("cpu")[:,-1])

                loop.set_description(f'Testing: ')

        scores = torch.cat(scores, dim=0).numpy().flatten()
