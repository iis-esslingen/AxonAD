...
class PCA(BaseDetector):
    ...
    def fit(self, X, y=None):
        ...
        # Compute distances in batches to avoid memory explosion
        batch_size = min(1000, X.shape[0])  # Process 1000 windows at a time
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            batch_scores = np.sum(
                cdist(X_batch, self.selected_components_) / self.selected_w_components_,
                axis=1
            )
            decision_scores_list.append(batch_scores)
            del X_batch, batch_scores  # Explicit cleanup
        
        self.decision_scores_ = np.concatenate(decision_scores_list).ravel()
        del decision_scores_list
    ...
    def decision_function(self, X):
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            decision_scores_list.append(batch_scores)
            del X_batch, batch_scores
        
        decision_scores_ = np.concatenate(decision_scores_list).ravel()
        del decision_scores_list
