...
def get_gpu(cuda):
    # Debug output
    print(f"[DEBUG] cuda parameter: {cuda}")
    print(f"[DEBUG] torch.cuda.is_available(): {torch.cuda.is_available()}")
    print(f"[DEBUG] hasattr(torch.backends, 'mps'): {hasattr(torch.backends, 'mps')}")
    ...
    elif cuda == True and hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
...
def cleanup_memory():
    """
    Comprehensive cleanup function for GPU/MPS memory.
    Call after model training/evaluation to free cached memory.
    """
    import gc
    ...
    # Another GC pass after cache clearing
    gc.collect()